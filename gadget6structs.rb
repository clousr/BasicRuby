...
class Gadget
  ...
  def install_app(name)
    ...
    @apps << app unless  @apps.include?(app)# only array, so could be added twice
  end
  def delete_app(name)
    app = apps.find { |installed| installed.name == name} #installed.name is a part of the Struct
    apps.delete(app) unless app.nil?#protection for if not there
  end
